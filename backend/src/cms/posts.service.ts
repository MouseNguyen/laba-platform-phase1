...
export class PostsService {
  ...
  private sanitizeContent(content: Record<string, any>): Record<string, any> {
    try {
      ...
      const sanitized = DOMPurify.sanitize(stringified, {
        ALLOWED_TAGS: ['p', 'h1', 'h2', 'h3', 'img', 'a', 'strong', 'em', 'ul', 'ol', 'li'],
   ...
   * CREATE POST
   */
  async create(dto: CreatePostDto, currentUserId?: number): Promise<PostResponseDto> {
    // Check slug exists
  ...
  async findBySlug(slug: string): Promise<PostResponseDto> {
    ...
    const result = this.mapToResponseDto(post);
  ...
  async update(id: number, dto: UpdatePostDto, currentUserId?: number): Promise<PostResponseDto> {
    ...
    if (dto.content !== undefined) {
      updateData.content = this.sanitizeContent(dto.content) as Prisma.InputJsonValue;
    }

    if (dto.authorId !== undefined) {
      updateData.author = dto.authorId ? { connect: { id: dto.authorId } } : { disconnect: true };
    }
  ...
  async restore(id: number, currentUserId?: number): Promise<PostResponseDto> {
    ...
    if (!existing) {
      throw new NotFoundException(`Post with ID ${id} is not deleted or does not exist`);
    }
   ...
   * BULK DELETE POSTS
   */
  async bulkDelete(ids: number[], currentUserId?: number): Promise<{ count: number }> {
    if (!ids || ids.length === 0) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (notFound.length > 0) {
        throw new NotFoundException(`Posts with IDs ${notFound.join(', ')} not found`);
      }
