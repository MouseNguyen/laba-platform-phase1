...
export class AuthRateLimitService {
  ...
  async checkLoginLimit(email: string, ip: string): Promise<void> {
    const perUserLimit = this.config.get<number>('RATE_LIMIT_LOGIN_PER_USER') ?? 10; // Increased from 5
    const perIpLimit = this.config.get<number>('RATE_LIMIT_LOGIN_PER_IP') ?? 50; // Increased from 10
  ...
  async checkRefreshLimit(ip: string): Promise<void> {
    const now = Date.now();
    const durationMs =
      (this.config.get<number>('RATE_LIMIT_REFRESH_DURATION_MINUTES') ?? 1) * 60 * 1000;
    ...
    if (!record || now >= record.resetAt) {
      ...
      if (this.refreshLimitStore.size >= MAX_TRACKED_IPS) {
        ...
        if (this.refreshLimitStore.size >= MAX_TRACKED_IPS) {
          ...
          if (this.refreshLimitStore.size >= MAX_TRACKED_IPS) {
            this.logger.warn(
              `[SECURITY] RefreshLimitStore reached MAX_TRACKED_IPS, emergency cleanup performed`,
            );
          }
        }
      }
