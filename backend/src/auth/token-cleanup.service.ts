...
export class TokenCleanupService {
    ...
    async cleanupUserTokens() {
        ...
        const graceDays =
            this.configService.get<number>('TOKEN_CLEANUP_GRACE_DAYS') ?? 30;
        const batchSize =
            this.configService.get<number>('TOKEN_CLEANUP_BATCH_SIZE') ?? 100;
        ...
        try {
            ...
            if (totalDeleted > 0) {
                this.logger.log(
                    `Cleanup finished. Deleted ${totalDeleted} tokens in ${duration}ms.`,
                );
            } else {
