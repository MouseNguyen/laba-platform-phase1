...
export class AuthService {
  ...
  async refresh(req: Request, res: Response) {
    ...
    if (tokenRecord.revoked_at) {
      ...
      if (timeSinceRevoke > gracePeriodMs) {
        ...
        this.logger.warn(`Token reuse detected for user ${tokenRecord.user_id}`);
        this.securityLogger.logAuthEvent('SESSION_COMPROMISED', detail);
        this.securityLogger.notifyAlertWebhook('SESSION_COMPROMISED', detail);
        ...
        this.logger.debug(`Token reuse within grace period for user ${tokenRecord.user_id}`);
      }
    }
    ...
    try {
      ...
      const newRefreshToken = await this.generateRefreshToken(tokenRecord.user, req);
  ...
  private async generateAccessToken(user: any): Promise<string> {
    ...
    const expiresIn = Number(this.configService.get('JWT_ACCESS_EXPIRES_IN')) || 900;
  ...
  private async generateRefreshToken(user: any, req: Request): Promise<string> {
    ...
    const refreshTtlSeconds =
      Number(this.configService.get('JWT_REFRESH_EXPIRES_IN')) || 7 * 24 * 60 * 60;
  ...
  private async hashPassword(password: string): Promise<string> {
    const memoryCost = Number(this.configService.get('ARGON2_MEMORY_COST')) || 19456;
    const timeCost = Number(this.configService.get('ARGON2_TIME_COST')) || 2;
    const parallelism = Number(this.configService.get('ARGON2_PARALLELISM')) || 1;
  ...
  private async handleLoginFailure(user: User): Promise<void> {
    const now = new Date();
    const threshold = this.configService.get<number>('ACCOUNT_LOCK_THRESHOLD') ?? 5;
    const baseLockMin = this.configService.get<number>('ACCOUNT_LOCK_DURATION_MIN') ?? 15;
  ...
  private async resetLoginLock(user: User): Promise<void> {
    if (!user.failed_login_attempts && !user.lock_until && !user.last_failed_attempt) {
      return;
    }
